  ...
  try {
    ...
    for (const key of Object.keys(data)) {
      for (let entry of data[key]) {
        ...
        const roleAccount = result.recordset[0].Role; // "Customer" hoặc "Seller"
        const profileTable = roleAccount + "Profile"; // "CustomerProfile" hoặc "SellerProfile"
        const profileId = roleAccount + "Id"; // "CustomerId" hoặc "SellerId"
